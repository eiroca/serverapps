{include file="header.tpl" pt="pg"}
  <p class="CENTER"><b>{$page->caption}</b>{br}</p>
  <p>{$page->aforisma.description}</p>
  {if ($page->kind_id == 1) || ($page->aforisma.author)}<p class="RIGHT">{if $page->aforisma.author}<i>({$page->aforisma.author})</i>{else}<i>(Anonimo)</i>{/if}</p>{/if}
